...
**为什么必须拆两个进程**：图库的全部意义是读用户的图片目录（默认 `D:/Pictures/gallery`），
...
| `gallery_config` | 查看/修改配置（图库根目录、扫描路径、缩略图尺寸、博客图片目录） |
...
尚未迁移（v1 有、v2 暂缺）：`gallery_markdown`（生成引用片段）、
`gallery_generate`（导出 HTML 画廊页）、`gallery_export`（导出索引）、
`gallery_sync`（同步到博客目录）。这些都要写外部路径，等路由层方案定下来再补。
...
```bash
node ../test-service.mjs        # 独立验证受管服务（不经 Hana）
node scripts/validate_app.mjs --dir .   # 官方结构校验
```

`test-service.mjs` 覆盖：服务启动、能力探测、外部目录访问、配置读写、
扫描、中英文检索、缩略图、取图、标签、索引重建、重启持久化。
